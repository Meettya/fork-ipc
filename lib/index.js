...
function executeOnChild(domain, services, message) {
  ...
  return _bluebird2.default.reject(Error('unknown execute domain: |' + message.domain + '| command: |' + message.command + '|'));
}
...
function doResponce(message) {
  if (requestQueue[message.id]) {
    if (message.status === STATUS.OK) {
      ...
      requestQueue[message.id](_bluebird2.default.reject(Error(message.error)));
    } else {
      requestQueue[message.id](_bluebird2.default.reject(Error('unknown status |' + message.status + '|')));
    }
  }
}
